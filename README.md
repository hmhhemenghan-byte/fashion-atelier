...
- Archive & Handoff 会将作品、细节图、过程档案、系列、编排关系、材料档案、色卡图片、Look 用料、技术包修订、尺寸规格、工艺说明与技术图、试身场次、版型问题、私密试身影像、最终封样、封样核对与证据、生产放行包、准备核对、生产偏差与处置时间线、设计评审及修改任务、Publication、保存的编辑排期、Private Showroom、专业回应、样衣借调、沟通留痕、实物资产、盘点记录、Placement 成果及证据图片、联系人、机会、互动待办、外联活动、对象审核、Editorial QA 和媒体对象清单合并为一份完整 JSON 交接数据包。
...
- 十五项事实关卡只使用明确的状态、数量与截止时间，逐项说明通过或阻塞原因；系列准备度也由“系列公开、Look 编排、Look 公开、发布包、发布安排”五个事实检查等权计算，不使用隐藏权重。
- 统一议程把逾期、今日、未来十四天和需要人工判断的事项排序后深链回原工作台；作战台本身只读，不自动改变状态、不发送外联，也不会向总览接口返回展厅令牌、邮箱或电话。
- 第 18 阶段本身不新增数据库表；当前 Studio Map 汇总 24 个业务工作台，完整归档结构已随第 25 阶段升级为 `nera-archive/16`。详细口径见 [`docs/phase-18-season-control-tower.md`](docs/phase-18-season-control-tower.md)。
...
- 评审台提供评审 CSV、修改任务 CSV 和完整 JSON 导出，并随当前 `nera-archive/16` 快照、SHA-256 校验和变更对比迁移。结构与接口见 [`docs/phase-19-atelier-review-board.md`](docs/phase-19-atelier-review-board.md)。
...
- 材料库、Look BOM 与完整 JSON 可分别导出；材料、用料关系与色卡媒体随当前 `nera-archive/16` 快照迁移。详细结构见 [`docs/phase-20-material-room.md`](docs/phase-20-material-room.md)。
...
- 技术包、尺寸规格、工艺说明可分别导出 CSV 或完整 JSON；全部记录、技术图对象键与快照数量随 `nera-archive/16` 迁移。详细结构见 [`docs/phase-21-technical-atelier.md`](docs/phase-21-technical-atelier.md)。
...
- 试身场次、版型问题、证据清单可分别导出 CSV 或完整 JSON，并随当前 `nera-archive/16` 的快照、摘要与变更对比迁移。详细结构见 [`docs/phase-22-fitting-room.md`](docs/phase-22-fitting-room.md)。
...
- 签核、核对与证据索引可分别导出 CSV 或完整 JSON，私密证据进入 R2 媒体清单；完整交接格式当前为 `nera-archive/16`。详细结构见 [`docs/phase-23-final-sample-gate.md`](docs/phase-23-final-sample-gate.md)。
...
- 放行包、准备核对可分别导出 CSV 或完整 JSON，并随当前 `nera-archive/16` 快照、摘要和变更对比迁移。详细结构见 [`docs/phase-24-production-release-desk.md`](docs/phase-24-production-release-desk.md)。
...
- 偏差主档、人工时间线可分别导出 CSV 或完整 JSON，并随 `nera-archive/16` 快照、摘要和变更对比迁移。详细结构见 [`docs/phase-25-production-change-control.md`](docs/phase-25-production-change-control.md)。
...
本地启动需要 Node.js `>=22.13.0`，执行 `npm ci` 后运行 `npm run dev`。本地示例环境变量见 `.env.example`。
